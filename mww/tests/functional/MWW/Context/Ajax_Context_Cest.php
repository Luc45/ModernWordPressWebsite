...
class Ajax_Context_Cest implements Context_Tests_Interface {
	public function test_is_rest_request( FunctionalTester $I ) {
		$code = <<<PHP
/** Plugin Name: Test */
add_action( 'init', function () {
	 echo (int) App\Service_Providers\Ajax_Providers\Ajax_Service_Provider::should_register();
	exit;
} );
PHP;
...
add_action( 'wp', function () {
	 echo sprintf('<div id="test-rest-response">%d</div>', (int) App\Service_Providers\Ajax_Providers\Ajax_Service_Provider::should_register());
	exit;
} );
PHP;
...
PHP;
		...
		$I->seeResponseEquals( '<div id="test-rest-response">1</div>' );
	}
}
